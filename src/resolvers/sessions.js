...
const createSessionMutation = (parentValue, { session: { name, blocks, participants } }, { auth }) =>
  SessionMgrService.createSession({ name, questionBlocks: blocks, participants, userId: auth.sub })

const modifySessionMutation = (parentValue, { id, session: { name, blocks, participants } }, { auth }) =>
  SessionMgrService.modifySession({ id, name, participants, questionBlocks: blocks, userId: auth.sub })
